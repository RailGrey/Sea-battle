...
a=2 #для того чтобы влезла всякая хуйня по горизонтали (какая именно, я уже не помню, АХАХАХАХАХХА)

d=2   
MaxPalubn=4

Dforx0=h/(2*intXboard+a)
Dfory0=h/(2*intYboard+a)
#D for table
D=int(min(Dforx0,Dfory0)) #int должен быть именно он, иначе ширина клеток будет непостоянной из-за округлений


if (w-intYboard*D)//2<0:y0=D
else: y0=int((w-intYboard*D)//2)
if (h-intXboard*D)//2<0:x0=D
else: x0=int((w-intXboard*D)//2)
...
y1=(w-D1*(MaxPalubn*3-1))//2#только для отрисовки интерфейса далее y1=y0 (избегать прямой подстановки D1, так как иначе из-за округления отступы сверху и снизу будут отличаться
x1=x0+h//2#отступы у карты противника
y2=y0
x2=x0
...
screen = pygame.display.set_mode((int(2*(intXboard*D+2*x0)), int(intYboard*D+2*y0)))
...
def shipplacement(Positions_List,deltaX,ShipsPositions,MaxPalubn): #Positions_List
...
PlayersshipplasmentStage=False #True ;FALSE ONLY FOR SPEED TEST!!!
    ...
    if TIMER=='start':
        ...
        if TIMER1==FPS//10:
            ...
            if TIMERtype=='Autoshipplacement':
                ...
                pygame.draw.rect(screen,(0,0,0),MenuAutoRectSelect, width=2)
                TIMER1=0
#Тут можно написать таймер как класс с разными параметрами, а не просто набр куи переменных      
        ...
        if event.type == pygame.QUIT:
            ...
            if   x1<=event.pos[0]<=x1+MaxPalubn*int(3/2*D1)+4*D1 and y1//2<=event.pos[1]<=y1//2+int(3*D1*MaxPalubn):           #блок кнопок
                ...
                if CountOstShips==[0 for i in range(MaxPalubn)]:
                    ...
                    shipplacement(Positions_list,x2,PlayersShips,MaxPalubn)
...
shipplacement=threading.Thread(target=shipplacement(Positions_list,x1,OppShips,MaxPalubn))
